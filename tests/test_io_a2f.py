...
Fixtures are written in the authentic layouts:
  EPW prefix.a2f : omega[meV], N a2F columns, N cumulative-lambda columns + footer
  QE   a2F.dos   : "frequencies in Rydberg" header, omega[Ry], total a2F, per-mode
"""
...
def _write_epw(tmp_path, name, omega, a2f_cols, smearings, lambda_scale=1.0):
    """Write a real-layout EPW prefix.a2f: omega, N a2F, N cumulative lambda + footer."""
    lam_cols = [_cumulative_lambda(omega, a) * lambda_scale for a in a2f_cols]
    cols = [omega] + list(a2f_cols) + lam_cols
    lines = ["".join(f"{c[i]:14.7f}" for c in cols) for i in range(omega.size)]
    footer = (
        " Integrated el-ph coupling\n  #  " + "  ".join(f"{c[-1]:.7f}" for c in lam_cols) + "\n"
        " Phonon smearing (meV)\n  #  " + "  ".join(f"{s:.7f}" for s in smearings) + "\n"
        " Fermi window (eV)  0.3\n"
    )
    p = tmp_path / name
    p.write_text("\n".join(lines) + "\n" + footer)
...
def _write_qe(tmp_path, name="a2F.dos", n_modes=2):
...
def test_epw_detection_columns_and_smearing(tmp_path):
    ...
    assert spec.n_columns == 3 and spec.n_smearings == 1 and spec.column == 2
    ...
    # cumulative-lambda column agrees -> no cross-check warning
...
def test_epw_multi_smearing_requires_choice(tmp_path):
    w, a2f = _pb_like(120)
    path = _write_epw(tmp_path, "sweep.a2f", w, [a2f, 0.9 * a2f], [0.1, 0.2])
    spec = read_a2f(path)
...
def test_epw_lambda_crosscheck_warns_on_mismatch(tmp_path):
    w, a2f = _pb_like(120)
    path = _write_epw(tmp_path, "pb.a2f", w, [a2f], [0.5], lambda_scale=2.0)  # inconsistent lambda col
    spec = read_a2f(path)
...
def test_no_signature_autodetect_requires_format(tmp_path):
    ...
    with pytest.raises(A2FColumnError) as e:
        read_a2f(str(p))  # no header/footer signature
...
def test_no_magnitude_fallback_low_energy_mev_not_rescaled(tmp_path):
    # A legitimate low-energy meV spectrum (omega_max < 1) forced as EPW must NOT
    # be rescaled by 13605 (the deleted magnitude heuristic would have).
    w = np.array([0.0, 0.2, 0.4, 0.6, 0.8])
    a = np.array([0.0, 0.3, 0.6, 0.3, 0.1])
    path = _write_epw(tmp_path, "lowe.a2f", w, [a], [0.05])
    spec = read_a2f(path, fmt="epw")
...
def test_negative_a2f_rejected_by_default_clamp_opt_in(tmp_path):
    ...
    spec = read_a2f(str(p), fmt="epw", clamp_negative=True)
    assert (spec.a2f >= 0).all() and spec.clamped_negative == 1
    assert spec.most_negative_a2f == pytest.approx(-0.05)
    assert any(w.code == "clamped_negative" for w in spec.warnings)


def test_clip_below_negative_is_param_error(tmp_path):
    w, a2f = _pb_like(60)
    path = _write_epw(tmp_path, "pb.a2f", w, [a2f], [0.5])
    with pytest.raises(A2FColumnError) as e:
        read_a2f(path, clip_below_mev=-1.0)
    assert e.value.code == "clip_negative" and e.value.exit_code == 4
